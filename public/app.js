(function () {
  ...
  function stTrex(c) {
    ...
    TR(c, [0.58, -0.23], [0.82, -0.14], [0.58, -0.05], "#9AD17B");
    TR(c, [-0.54, 0.1], [-0.9, -0.04], [-0.56, 0.28], "#9AD17B");
    for (var i = 0; i < 4; i++) TR(c, [-0.44 + i * 0.2, -0.18], [-0.34 + i * 0.2, -0.42], [-0.24 + i * 0.2, -0.18], "#C6E58A");
    bead(c, 0.26, -0.23, 0.055);
    cheek(c, 0.5, -0.09, 0.06);
    grin(c, 0.42, -0.06, 0.11);
  ...
  function stPtero(c) {
    ...
    bead(c, -0.08, -0.43, 0.045); bead(c, 0.08, -0.43, 0.045);
    cheek(c, -0.18, -0.34, 0.045); cheek(c, 0.18, -0.34, 0.045);
    grin(c, 0, -0.32, 0.045);
  ...
  function stSeahorse(c) {
    ...
    E(c, 0.18, -0.58, 0.26, 0.2, "#7EC9D4");
    TR(c, [0.36, -0.58], [0.68, -0.52], [0.36, -0.44], "#7EC9D4");
    for (var i = 0; i < 6; i++) TR(c, [-0.18, -0.5 + i * 0.15], [-0.42, -0.44 + i * 0.15], [-0.18, -0.36 + i * 0.15], "#F6C6D5");
    c.beginPath(); c.arc(0.05, 0.42, 0.28, 1.55 * Math.PI, 0.25 * Math.PI); c.stroke();
    bead(c, 0.19, -0.62, 0.05);
    cheek(c, 0.38, -0.48, 0.05);
    grin(c, 0.3, -0.44, 0.035);
  }
  function stJelly(c) {
  ...
  function stDolphin(c) {
    ...
    E(c, 0.02, 0.2, 0.22, 0.08, "#D9F2FF", -0.15);
  }
  ...
  function stReindeer(c) {
    oc(c, "rgba(120,85,55,0.55)");
    ground(c);
    c.beginPath(); c.moveTo(-0.24, -0.4); c.lineTo(-0.38, -0.72); c.moveTo(-0.38, -0.62); c.lineTo(-0.52, -0.78); c.moveTo(-0.38, -0.62); c.lineTo(-0.24, -0.76);
    c.moveTo(0.24, -0.4); c.lineTo(0.38, -0.72); c.moveTo(0.38, -0.62); c.lineTo(0.52, -0.78); c.moveTo(0.38, -0.62); c.lineTo(0.24, -0.76); c.stroke();
  ...
  var STAMPS = [
    ...
    ["Rexi", "t-rex", stTrex],
    ["Trixie", "triceratops", stTricera],
    ["Spike", "stegosaurus", stStego],
    ["Ptera", "pterodactyl", stPtero],
    ["Marina", "mermaid", stMermaid],
    ["Coral", "seahorse", stSeahorse],
    ["Jelli", "jellyfish", stJelly],
    ["Splash", "dolphin", stDolphin],
    ["Patch", "pumpkin", stPumpkin],
    ["Boo", "ghost", stGhost],
    ["Miso", "witch cat", stWitchCat],
    ["Nox", "bat", stBat],
    ["Rudy", "reindeer", stReindeer],
    ["Flurry", "snowman", stSnowman],
    ["Piney", "christmas tree", stTree],
    ["Noel", "santa bear", stSantaBear]
  ];
